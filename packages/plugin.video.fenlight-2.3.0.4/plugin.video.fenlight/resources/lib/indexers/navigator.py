...
class Navigator:
	...
	def sevenplus(self):
		...
		for item in sp.menu_items():
			params = {'mode': 'sevenplus.dispatch', '_': item.get('route', ''), 'isFolder': item.get('isFolder', 'true')}
			for key in ('slug',):
				if key in item: params[key] = item[key]
			self.add(params, item['label'], item.get('icon', '7plus'))
		self.end_directory()
