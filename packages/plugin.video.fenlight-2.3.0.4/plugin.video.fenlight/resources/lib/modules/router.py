...
def routing(sys):
	...
	if 'navigator.' in mode:
	...
	elif 'slzprovision' in mode:
		from modules import slz_provision
		if mode == 'slzprovision.request_claim':
			return slz_provision.request_provisioning_claim()
	elif 'sevenplus' in mode:
		from modules import sevenplus
		if mode == 'sevenplus.dispatch':
			return sevenplus.dispatch(params)
		elif mode == 'sevenplus.install_dependencies':
			return sevenplus.install_dependencies()
		elif mode == 'sevenplus.open_settings':
			return sevenplus.open_settings()
		...
		updater_mode = mode.split('.')[1]
		if updater_mode == 'update_check' and 'action' in params:
			return updater.update_check(int(params['action']))
		return exec('updater.%s()' % updater_mode)
	##EXTRA modes##
	elif mode == 'set_view':
